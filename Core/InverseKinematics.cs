...
public class Limb
{
    ...
    public Limb(int size, Vector2 basePosition, bool fixedBase, float segmentLength, float lengthStep = 0, float strokeWeight = 1f,
        float strokeWeightStep = 0, Texture2D texture = null)
    {
    ...
    public void Update()
    {
        for (int i = _segments.Length - 2; i >= 0; i--) _segments[i].Follow(_segments[i + 1].Start);

        if (FixedBase)
            _segments[0].Start = BasePosition;

        for (int i = 1; i < _segments.Length; i++) _segments[i].Start = _segments[i - 1].End;
    ...
    {
        foreach (Bone segment in _segments) segment.Draw(_texture, color);
    }
}